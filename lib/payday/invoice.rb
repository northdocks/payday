module Payday
  ...
  class Invoice
    ...
    attr_accessor :invoice_number, :bill_to, :ship_to, :notes, :line_items, :shipping_fee, :shipping_description,
    ...
    def initialize(options =  {})
      ...
      self.shipping_fee = options[:shipping_fee] || nil
    ...
    # Shipping rate
    def shipping_fee=(value)
      case value
      when Money
        @shipping_fee = value
      when NilClass
        @shipping_fee = Money.new(0, currency)
      else
        @shipping_fee = Money.new(value, currency)
      end
    end

    def currency
      @currency || Payday::Config.default.currency
    end
  end
end