...
module Payday::Invoiceable
  ...
  def subtotal
    line_items.inject(Money.new(0, currency)) { |result, item| result += item.amount }
  end
  ...
  def tax
    if defined?(tax_rate)
      ...
      return Money.new(0, calculated.currency) if calculated < Money.new(0, calculated.currency)
      calculated
    else
      0
    end
